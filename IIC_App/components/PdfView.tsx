...
export const PdfView: React.FC<Props> = ({ 
  ...
  const handlePdfClick = (type: 'FREE' | 'PREMIUM' | 'ULTRA') => {
      ...
      const contentId = `${chapter.id}_pdf_${type}`;
  ...
  const confirmPayment = () => {
      ...
      const contentId = `${chapter.id}_pdf_${confirmModal.type}`;
      ...
      const updatedUser = { 
          ...user, 
          credits: user.credits - confirmModal.price,
       ...
       {activePdf ? (
           <div className="h-[calc(100vh-80px)] w-full bg-slate-100 relative">
               <iframe 
                   ...
                   title="PDF Viewer"
               ></iframe>
               {/* OVERLAY for Top Bar Protection */}
               <div className="absolute top-0 left-0 w-full h-14 bg-transparent z-20" onClick={(e) => { e.preventDefault(); e.stopPropagation(); }} />
           </div>
       ) : (
