...
import { getChapterData, saveUserToLive, subscribeToChapterData } from '../firebase';
...
export const VideoPlaylistView: React.FC<Props> = ({ 
  ...
  const [confirmModal, setConfirmModal] = useState<{index: number, price: number, videoId: string} | null>(null);
  ...
  useEffect(() => {
    ...
    const unsub = subscribeToChapterData(key, (data) => {
  ...
  const handleVideoClick = (index: number) => {
      ...
      // 1. Check if Admin
      if (user.role === 'ADMIN') {
          setActiveVideo({...video, id: `vid_${index}`});
          return;
      }
      ...
      const contentId = `${chapter.id}_video_${index}`;
      if (user.unlockedContent?.includes(contentId)) {
          setActiveVideo({...video, id: contentId});
          return;
      }
      ...
      setConfirmModal({ index, price, videoId: contentId });
  };

  const confirmPayment = () => {
      if (!confirmModal) return;
      const { index, price, videoId } = confirmModal;
      ...
      const updatedUser = { 
          ...
          unlockedContent: [...(user.unlockedContent || []), videoId]
      };
      ...
      setActiveVideo({ ...playlist[index], id: videoId });
      setConfirmModal(null);
  };
       ...
       <div className="aspect-video bg-black w-full sticky top-[73px] z-10 relative">
           {activeVideo ? (
               ...
               <iframe 
                   ...
                   title="Video Player"
               />
               {/* OVERLAY TO PREVENT CLICKING OUT (Especially for YouTube Title) */}
               <div className="absolute top-0 left-0 w-full h-12 bg-transparent z-20" onClick={(e) => { e.preventDefault(); e.stopPropagation(); }} />
               </>
           ) : (
