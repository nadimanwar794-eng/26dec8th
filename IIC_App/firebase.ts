...
export const subscribeToChapterData = (key: string, callback: (data: any) => void) => {
    // Priority: Firestore Realtime Listener (Better for complex data)
    // Fallback: RTDB Listener
    ...
    const unsubFirestore = onSnapshot(doc(db, "content_data", key), (docSnap) => {
        if (docSnap.exists()) {
            callback(docSnap.data());
             ...
             // If document doesn't exist in Firestore, maybe it's in RTDB (legacy or different path)
             // We can attach RTDB listener here as fallback, or just return null
             ...
             }, { onlyOnce: true }); // Use once to avoid double firing if both exist? No, we want realtime.
             // But managing two listeners is tricky. 
             // Let's assume content is primarily in Firestore now based on `saveChapterData` dual write.
             callback(null);
        }
    });
