...
export const AdminDashboard: React.FC<Props> = ({ onNavigate, settings, onUpdateSettings, onImpersonate, logActivity }) => {
  ...
  const [localSettings, setLocalSettings] = useState<SystemSettings>(settings || {
      ...
      wheelProbabilities: [40, 30, 20, 10], // Default probabilities
                  ...
                  {activeTab === 'CONFIG_GAME' && (
                       <div className="bg-slate-50 p-4 rounded-xl border border-slate-200 space-y-4">
                           ...
                           <div className="pt-4 border-t border-slate-200">
                               <p className="font-bold text-slate-700 mb-2 text-sm">Rewards & Probabilities</p>
                               <p className="text-xs text-slate-500 mb-2">Comma separated values. Must have same number of items.</p>
                               
                               <div className="space-y-3">
                                   <div>
                                       <label className="text-xs font-bold text-slate-500 uppercase block mb-1">Rewards (Credits)</label>
                                       <input 
                                          type="text" 
                                   ...
                                   <div>
                                       <label className="text-xs font-bold text-slate-500 uppercase block mb-1">Probabilities (Weights)</label>
                                       <input 
                                          type="text" 
                                          value={localSettings.wheelProbabilities?.join(',') || ''} 
                                          onChange={e => setLocalSettings({...localSettings, wheelProbabilities: e.target.value.split(',').map(Number)})} 
                                          className="w-full p-2 border rounded-lg" 
                                          placeholder="100, 50, 20, 5, 1"
                                       />
                                       <p className="text-[9px] text-slate-400">Higher number = Higher chance.</p>
                                   </div>
                               </div>
                           </div>
                       </div>
